...
class StudyTimeLogCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.updating_members = []

    @commands.Cog.listener()
    async def on_ready(self):
        await self.bot._fully_ready.wait()
        self.bot.module_count += 1
        print(f"{self.bot.module_count}. Study time log module ready")
    ...
    async def on_voice_state_update(
        ...
        if not member_before.channel and member_after.channel:
            ...
            while member.id in self.updating_members:
                await asyncio.sleep(1)
            self.updating_members.append(member.id)
            ...
            # if user not in users database
                ...
                # Call the on_member_join handler
                await self.bot.get_cog("OnMemberJoinCog").on_member_join(member=member)
                await UserStudySection(
                    user=await Users.find_one(Users.discord_id == member.id),
                    start_study_time=Now().now,
                ).insert()
        ...
        # member leave channel
            ...
            if member.id in self.updating_members:
                self.updating_members.remove(member.id)
...
async def setup(bot):
    await bot.add_cog(StudyTimeLogCog(bot))
