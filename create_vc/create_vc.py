from base import (
    ...
    homie_id,
    easter_eggs_id)
from feature_func.easy_mongodb import db
...
@bot.listen()
async def on_ready():
  global guild,guild_ids,everyone_id
        ...
        if check_avaiable_name(member.name) == False:
          ...
          try:
            ...
            if voice_channel_after.id == 918549426732142653:
              category=bot.get_channel(900439704950956053)
              db[mem_id]["locate"]="sg"
              lim = 15

            elif voice_channel_after.id == 918549341948497961:
              category=bot.get_channel(901518444271386694)
              db[mem_id]["locate"]="cp"
              lim = 2

            elif voice_channel_after.id == 918549182107746356:
              category=bot.get_channel(900598666572750929)
              db[mem_id]["locate"]="sa"
              lim = 1

            elif voice_channel_after.id == 922461169799790642:
              category=bot.get_channel(915512539733975050)
              db[mem_id]["locate"]="cr"
              lim = 0
            elif voice_channel_after.id == 923964509935243265:
              category=bot.get_channel(902499044356657173)
              db[mem_id]["locate"]="ts"
              lim = 0
    ...
    if str(voice_channel_before.id) in db.keys():
      ...
      if voice_channel_before.members == []:
        ...
        if vc in db.keys():
          ...
          channel_del = bot.get_channel(int(vc))
          if channel_del != None:
            await channel_del.delete()          
          channel_del = bot.get_channel(text_channel)
          if channel_del != None:
            await channel_del.delete() 
        ...
        cc_channel = get(bot.get_all_channels(), id=db[vc]["cc_id"] )
...
##member in    
    ...
    if str(voice_channel_after.id) in db.keys():
          can_clear = False
          cc_channel = get(bot.get_all_channels(), id=db[str(voice_channel_after.id)]["cc_id"] )
          if cc_channel != None:
            overwrite = discord.PermissionOverwrite()
            overwrite.view_channel=True
            overwrite.send_messages=True
...
async def clean(ctx):
  ...
  if can_clear == False or ctx.channel.id != 884447828330549349:
    ...
    async with ctx.typing():
      ...
      bot.loop.create_task(fix_before_start())
...
async def _public(ctx:SlashContext):
  if str(ctx.author.id) in db.keys():
    vc_channel = get(bot.get_all_channels(), id=db[str(ctx.author.id)]["vc_id"] )      
    overwrite = discord.PermissionOverwrite()
    overwrite.connect=True
      ...
      for key in member_list:
        ...
        if "cc_id" in db[key]:
          ...
          if member == None:
            ...
            vc_channel = get(bot.get_all_channels(), id=db[key]["vc_id"]) 
            if vc_channel != None:
...
########clear cc
      for key in cc_list:
        check = False
        vc_channel = get(bot.get_all_channels(), id=db[key]["vc_id"])   
        cc_channel = get(bot.get_all_channels(), id=int(key)) 
        if vc_channel != None:
          if vc_channel.members == []:
            if cc_channel != None:
...
########clear vc
      for key in vc_list:
        check = False
        vc_channel = get(bot.get_all_channels(), id=int(key))   
        if vc_channel != None:
          if vc_channel.members == []:
